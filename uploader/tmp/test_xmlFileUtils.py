...
def test_ctor():
   ...
   print("--- test_ctor")
   fullPath = "testData/inferno-threeLines.eaf"
...
def test_validateXML():
   ...
   assert(result["details"] == expectedDetails)
     ...
     #----------------------------------------------------------
   fullPath = "testData/badMexico.xml"
   print("    %s" % fullPath)
   contents = createBase64EncodedStringFromEAFfile(fullPath)
   projectDirectory = "tmp"
   utils = XmlFileUtils(fullPath, projectDirectory, contents, verbose=True)
   ...
   expectedDetailsBeginsWith = 'invalid xml file badMexico.xml : failed validating <Element \'observationResults\''
   assert(result["details"].find(expectedDetailsBeginsWith) >= 0)

     #----------------------------------------------------------
     # one misspelled tag in an otherwise valid document
     #----------------------------------------------------------
   fullPath = "testData/inferno-misspelledTag.eaf"
   print("    %s" % fullPath)
   contents = createBase64EncodedStringFromEAFfile(fullPath)
   projectDirectory = "tmp"
   utils = XmlFileUtils(fullPath, projectDirectory, contents, verbose=True)
   ...
   assert(x.find("Reason: Unexpected child with tag \'TIME_ORDERxxx\'") >= 0)
     ...
     #----------------------------------------------------------
   fullPath = "testData/inferno-misnamedTierType.eaf"
   print("    %s" % fullPath)
   contents = createBase64EncodedStringFromEAFfile(fullPath)
   projectDirectory = "tmp"
   utils = XmlFileUtils(fullPath, projectDirectory, contents, verbose=True)
   ...
   fullPath = "testData/inferno-misnamedParentRef.eaf"
   print("    %s" % fullPath)
   contents = createBase64EncodedStringFromEAFfile(fullPath)
   projectDirectory = "tmp"
   utils = XmlFileUtils(fullPath, projectDirectory, contents, verbose=True)
